...
class TestSaveStep:
    ...
    def test_save_excel_output_with_include_flag(self, mock_atomic):
        ...
        assert args[0] == "output.xlsx"  # original_path
        assert args[2] is False  # create_backup
    ...
    def test_save_excel_output_with_backup(self, mock_atomic):
        ...
        mock_atomic.assert_called_once()
        assert mock_atomic.call_args[0][2] is True  # create_backup
    ...
    def test_save_pdf_output_success(self, mock_atomic):
        ...
        mock_atomic.assert_called_once()
        args = mock_atomic.call_args[0]
        assert args[0] == "output.pdf"  # original_path
        assert args[2] is False  # create_backup
    ...
    def test_complete_workflow_integration(self, mock_atomic):
        ...
        # Verify backup creation was requested for both
        excel_call = mock_atomic.call_args_list[0]
        pdf_call = mock_atomic.call_args_list[1]

        assert excel_call[0][2] is True  # create_backup for Excel
        assert pdf_call[0][2] is True  # create_backup for PDF
