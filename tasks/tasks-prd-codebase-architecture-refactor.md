...
- [ ] 1.0 Establish core architecture and interfaces (Protocols, models)
  - [ ] 1.1 Create package layout: `core/`, `adapters/`, `utils/`, `io/` (and keep Tkinter UI).
  - [ ] 1.2 Add `core/interfaces.py` with Protocols: `ExcelRepo`, `PdfRepo`, `Logger`.
  - [ ] 1.3 Add `core/models.py` with `Options`, `Result`, `Bookmark`, `PageRange`, `Record` dataclasses.
  - [ ] 1.4 Ensure all new modules use top-level imports, type hints, and docstrings.

- [ ] 2.0 Extract pure Excel transforms and adapters; update Excel formatting
  - [ ] 2.1 Create `core/transform/excel.py` for: clean types, sort_and_renumber, original index mapping.
  - [ ] 2.2 Implement `adapters/excel_repo.py` to load/save Excel and call `ExcelFormatter`.
  - [ ] 2.3 Update `excel_formatter.py` to use `utils/excel_utils` and remain formatting-only.
  - [ ] 2.4 Replace duplicate column/date logic with shared utils.
  - [ ] 2.5 Verify output parity on fixture Excel (content-identical checks).

- [ ] 3.0 Extract PDF adapters and pure bookmark transforms
  - [ ] 3.1 Create `core/transform/pdf.py` for: title generation, extract index, page range detection.
  - [ ] 3.2 Implement `adapters/pdf_repo.py` for read/copy/write and outline handling.
  - [ ] 3.3 Keep natural sort behavior in transforms; writer only writes.
  - [ ] 3.4 Verify bookmark/title parity on fixture PDF (content-identical titles/order).

- [ ] 4.0 Implement ValidationService and integrate with orchestrator
  - [ ] 4.1 Implement `core/services/validate.py` composing existing validators.
  - [ ] 4.2 Treat required header and required-duplicate issues as hard errors.
  - [ ] 4.3 Treat PDF bookmark page conflicts as hard errors.
  - [ ] 4.4 Raise domain exceptions (no UI code); map to existing messages.

- [ ] 5.0 Implement RenumberService and thin Tkinter UI adapter
  - [ ] 5.1 Implement `core/services/renumber.py` to orchestrate load/validate/transform/save.
  - [ ] 5.2 Add a `Logger` adapter to pipe logs to Tkinter status area.
  - [ ] 5.3 Refactor `abstract_renumber.py` to build `Options` and call `RenumberService`.
  - [ ] 5.4 Preserve sheet selection UX and existing dialogs/messages.

- [ ] 6.0 Add DRY utilities (excel_utils, dates) and refactor call sites
  - [ ] 6.1 Implement `utils/excel_utils.py` with index↔letter helpers.
  - [ ] 6.2 Implement `utils/dates.py` with `parse_robust` and `format_mdy` helpers.
  - [ ] 6.3 Remove duplicate implementations in existing modules.

- [ ] 7.0 Implement backups/atomic save helpers and integrate
  - [ ] 7.1 Implement `io/files.py` for backups and atomic Excel save.
  - [ ] 7.2 Update Excel adapter to use atomic save and backup helpers.

- [ ] 8.0 Ensure functional parity with golden fixtures; add unit tests
  - [ ] 8.1 Add small Excel/PDF fixtures and expected outputs (goldens).
  - [ ] 8.2 Add unit tests for `core/transform/*` and `core/services/validate.py`.
  - [ ] 8.3 Add smoke tests for adapters against fixtures.
  - [ ] 8.4 Achieve ≥ 80% coverage on core transforms/services.
