...
class BuildResult:
    ...
    def __str__(self) -> str:
        ...
        if self.success:
            size_mb = self.file_size_bytes / (1024 * 1024) if self.file_size_bytes else 0
...
class BuildOrchestrator:
    ...
    def __init__(
        ...
        if optimize not in ('basic', 'medium', 'aggressive'):
            raise ValueError(f"Invalid optimize: {optimize}. Must be 'basic', 'medium', or 'aggressive'")
    ...
    def validate_prerequisites(self) -> tuple[bool, list[str]]:
        ...
        try:
            ...
            if free_mb < 500:
                errors.append(f"Insufficient disk space. Need 500MB, have {free_mb:.0f}MB")
        except Exception as e:
            errors.append(f"Cannot check disk space: {e}")
        
        # Platform warning
        if platform.system() != 'Windows' and not errors:
            print("[⚠] Warning: Building on non-Windows platform. Test on Windows required.")
    ...
    def build(self) -> BuildResult:
        ...
        try:
            ...
            if not self.verbose and result.stdout:
                ...
                important_lines = [l for l in lines if 'successfully' in l.lower() or 'warning' in l.lower()]
                if important_lines:
        ...
        if not exe_path:
            error_msg = self._format_error(
                ...
                f"Expected executable in dist/ directory but none found",
    ...
    def _format_error(self, category: str, brief: str, details: str, solution: str) -> str:
        """
...
Examples:
  ...
  %(prog)s --verbose                          # Verbose output
    ...
    parser.add_argument(
        '--verbose',
        action='store_true',
        help='Show detailed PyInstaller output'
    )
